...
auto print_search_status(Database &database, const Args &args, const std::string &database_path, const TableSpec &table, const SearchResult &result) -> void {
    std::ignore = args;
    ...
    if (stats.successful()) {
        ...
        std::cout << std::format(
            "Magic number: 0x{:016x} | Index bits: {:2d} | Max index: {:5d}{} {:5d} | Constr. "
            "coll.: {:3d} | Tries: {}\n",
            result.magics.magic_number, 64 - result.magics.shift, table_stats.max_index, better_result ? '*' : ' ', result.generator_result.expected_entries,
        ...
        if (stats.expected_entries != 0) {
            std::cout << std::format(" = {:.2f} %", static_cast<double>(stats.stored_entries) * 100.0 / static_cast<double>(stats.expected_entries));
        }
        std::cout << std::format("\n  Max index: {}\n", stats.max_index);
